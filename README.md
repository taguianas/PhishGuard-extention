...
| 3 | Domain impersonation — Levenshtein ≤ 2 from a known brand | +35 |
...
PhishGuard detects all 11 Unicode bidirectional formatting characters that have no legitimate use inside a URL. The most dangerous, `U+202E` (Right-To-Left Override), forces text to render right-to-left — making `moc.rekcah\u202Eelgoog` display as `googlehacker.com`.
...
│
└── CLAUDE.md              Development conventions (AI-assisted workflow)
```
...
Click the PhishGuard toolbar icon → **Settings**.
...
- **Detection log** — up to 200 entries with:
  - Timestamp, full URL (truncated), domain
  - Risk level badge and score
  - Domain age tag (if RDAP was checked)
  - All triggered indicator labels
